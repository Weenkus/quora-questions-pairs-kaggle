...
def prediction_correction(predictions, p=0.18):
    a = p / 0.37
    b = (1 - p) / (1 - 0.37)
...
def main():
    ...
    generate_submission(predictions, 'xgb15_nO_nC.csv')

    print('Generating predictions with correction')
    predictions = prediction_correction(predictions)
    generate_submission(predictions, 'xgb158_nO_yC.csv')
