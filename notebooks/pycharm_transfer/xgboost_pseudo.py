...
def main():
    ...
    print('Generating psuedo labels')
    y_psuedo_test = np.array(model.predict(X_test))

    X_pseudo = np.vstack([X_train, X_test])
    print(len(y_train), len(y_psuedo_test))
    y_pseudo = np.vstack([y_train, y_psuedo_test])
    ...
    generate_submission(predictions, 'xgb1_pseudo_yO_nC')

    print('Generating predictions with correction')
    predictions = prediction_correction(predictions)
    generate_submission(predictions, 'xgb1_pseudo_yO_yC')
